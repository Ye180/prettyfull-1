...
const inputStyle = { width: '100%', padding: '12px', marginBottom: '15px', border: '1px solid #ccc', borderRadius: '4px', boxSizing: 'border-box', fontSize: '1em', backgroundColor: '#fff' }; // Fond blanc pour visibilité
const previewContainerStyle = { marginTop: '10px', textAlign: 'center' };
const previewImageStyle = { maxWidth: '150px', maxHeight: '100px', border: '1px solid #eee', borderRadius: '4px', objectFit: 'contain' };
const currentImageStyle = { ...previewImageStyle, opacity: 0.7 };
const buttonStyle = { padding: '10px 20px', backgroundColor: '#57a481', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '0.9em', fontWeight: 'bold' }; // Utilise .rounded-lg de Tailwind implicitement
const disabledButtonStyle = { ...buttonStyle, backgroundColor: '#a3a3a3', cursor: 'not-allowed' };
const discardButtonStyle = { ...buttonStyle, backgroundColor: '#f3f4f6', color: '#dc2626', border: '1px solid #e5e7eb' }; // Style pour Discard
const dropzoneStyle = {
    width: '100%', minHeight: '150px', borderRadius: '0.75rem', border: '2px dashed #d1d5db',
    backgroundColor: '#f9fafb', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', cursor: 'pointer', padding: '1rem'
};
...
const ModalAddCategory = ({ show, handleClose, categoryData, onSaveSuccess, apiBaseUrl, ...props }) => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            ...
            try {
                ...
                setCategories(categoryData ? data.filter(cat => (cat._id || cat.id) !== (categoryData._id || categoryData.id)) : data);
            } catch (error) {
    ...
    }, [preview]); // Inclure preview pour le nettoyage
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(apiUrl, { method: apiMethod, body: formDataApi });
            if (!response.ok) {
    ...
    return (
        <Fragment>
            <Modal show={show} onHide={onHideClick} centered size="lg" {...props}> {/* Utilise show et onHideClick */}
                 <Modal.Header closeButton> {/* Utilise le header standard de react-bootstrap */}
                    <Modal.Title>{isEditing ? "Modifier la Catégorie" : "Ajouter une Nouvelle Catégorie"}</Modal.Title>
                 </Modal.Header>
                <Modal.Body>
                    ...
                    <div className="p-4">
                        <Form onSubmit={handleSubmit}>
                            ...
                            <Form.Group controlId="categoryImage" className="mb-4">
                                <Form.Label style={labelStyle}>Image {isEditing ? "(Optionnel : Remplacer)" : "(Optionnel)"}</Form.Label>
                                <div {...getRootProps({ style: { ...dropzoneStyle, ...(isDragActive ? dropzoneActiveStyle : {}) } })}>
                                    <input {...getInputProps()} />
                                    <div className="flex flex-col items-center justify-center">
                                         {/* Affichage preview ou image actuelle DANS le dropzone */}
                                         {preview ? (
                                             <img src={preview} alt="Prévisualisation" style={previewImageStyle} className="mb-2"/>
                                         ) : currentImageUrl ? (
                                             <img src={`${apiBaseUrl}${currentImageUrl}`} alt="Actuelle" style={currentImageStyle} className="mb-2" />
                                         ) : (
                                            <img src={originalUrl + "/images/FileImage.svg"} alt="Upload Icon" className="w-10 h-10 mb-3" />
                                         )}

                                        {isDragActive ? (
                                            <p className="text__12 text-blue-600">Déposez l'image ici...</p>
                                        ) : (
                                            <>
                                                <p className="text__12 text-gray-500 mb-1">
                                                    {imageFile ? imageFile.name : (isEditing && currentImageUrl ? "Glissez-déposez ou cliquez pour remplacer" : "Glissez-déposez une image ou cliquez pour sélectionner")}
                                                </p>
                                                {!imageFile && !(isEditing && currentImageUrl) && <p className="text-blue-600 font-medium text__14">Cliquez pour Uploader</p>}
                                            </>
                                        )}
                                    </div>
                                </div>
                            ...
                            <Form.Group controlId="categoryName" className="mb-3">
                                <Form.Label style={labelStyle} className="text-[#A3A3A3]"> {/* Ajout style label */}
                                    Nom Catégorie *
                                </Form.Label>
                                <Form.Control
                            ...
                            <Form.Group controlId="categoryDescription" className="mb-3">
                                ...
                                <div className="textareaStyle hCategory flex flex-col-reverse border border-Mborderborderprimary rounded-lg focus-within:border-Mborderborderprimary">
                                    <ReactQuill
                                        theme="snow"
                                        value={description}
                            ...
                            <div className="mt-4 flex items-center gap-2 justify-end">
                                ...
                                <button
                                    ...
                                    style={loading ? disabledButtonStyle : buttonStyle}
                                     className="px-3 py-2 rounded-lg" // Utilise tes classes si tu préfères
                                >
                                    {loading ? 'Sauvegarde...' : (isEditing ? 'Mettre à jour' : 'Ajouter Catégorie')}
                                </button>
                            </div>
                        </Form>
                    </div>
