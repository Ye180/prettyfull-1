import { ERROR_CODES, type ErrorCode } from "@prettyfull/contracts";
...
export const notFound = (resource = "Ressource") =>
	new AppError(ERROR_CODES.NOT_FOUND, `${resource} introuvable.`, 404);
...
export const insufficientStock = (
	message: string,
	details?: Record<string, string[]>,
) => new AppError(ERROR_CODES.INSUFFICIENT_STOCK, message, 409, details);

/** Violation de la règle de cohérence simple / à variantes (§2.2). */
export const invalidProductModel = (message: string) =>
	new AppError(ERROR_CODES.INVALID_PRODUCT_MODEL, message, 422);

export const paymentError = (message: string, details?: Record<string, string[]>) =>
	new AppError(ERROR_CODES.PAYMENT_ERROR, message, 402, details);
