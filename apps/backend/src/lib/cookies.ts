...
export type CookieScope = keyof typeof REFRESH_COOKIES;
...
export const setRefreshCookie = (
	...
	setCookie(
		c,
		REFRESH_COOKIES[scope],
		token,
		baseOptions(env.JWT_REFRESH_TTL_SECONDS),
	);
};

export const readRefreshCookie = (c: Context, scope: CookieScope): string | undefined =>
	getCookie(c, REFRESH_COOKIES[scope]);

export const clearRefreshCookie = (c: Context, scope: CookieScope): void => {
