 ...
 */
const skuBase = (slug: string): string => slug.toUpperCase().replaceAll("-", "");
...
const seedRbac = async (): Promise<Map<RoleKey, string>> => {
	...
	const roleIdByKey = new Map(
		roleRows.map((row) => [row.key as RoleKey, row.id] as const),
	);

	const links = ROLE_KEYS.flatMap((roleKey) =>
		ROLE_PERMISSIONS[roleKey].flatMap((permissionKey) => {
	...
	console.log(
		`  RBAC : ${permissionRows.length} permissions, ${roleRows.length} rôles, ${links.length} liaisons`,
	);
	return roleIdByKey;
};
...
const seedUsers = async (roleIdByKey: Map<RoleKey, string>): Promise<string> => {
	...
	const staffSeeds: { email: string; firstName: string; lastName: string; role: RoleKey }[] = [
	...
	console.log(
		`  utilisateurs : ${staffRows.length} back-office + 1 cliente (mot de passe « ${DEMO_PASSWORD} »)`,
	);
	return customer!.id;
};
...
const seedSettingsAndIntegrations = async (adminId: string): Promise<void> => {
...
const seedCatalog = async (adminId: string) => {
	const [root] = await db
		.insert(t.categories)
		.values({
	...
	const stockKey = (product: string, variant: string | null, size: string | null) =>
	...
	for (const seed of SEED_PRODUCTS) {
		...
		await db.insert(t.productImages).values(
			seed.images.map((url, position) => ({ productId, url, alt: seed.name, position })),
		);
	...
	const openingMovements = inventoryRows
		...
		.map((row) => ({
			...
			reason: "supplier_receipt" as const,
	...
	const inventoryIdByKey = new Map(
		inventoryRows.map((row) => [stockKey(row.productId, row.variantId, row.sizeId), row.id]),
	);

	console.log(
		`  catalogue : ${SEED_CATEGORIES.length + 1} catégories, ${SEED_PRODUCTS.length} produits, ${inventoryRows.length} points de stock`,
	);
...
const seedVariants = async (
	...
	for (const [variantIndex, variant] of seed.variants.entries()) {
		...
		await db.insert(t.variantImages).values(
		...
		const sizeRows = await db
			.insert(t.sizes)
			.values(
				variant.sizes.map((label, position) => ({
					variantId,
					label,
