...
adminCatalogRoutes.get(
	...
	async (c) => {
		...
		if (query.tree) return c.json(await categories.getCategoryTree());
...
adminCatalogRoutes.post(
	"/categories/reorder",
	requirePermission(PERMISSIONS.catalog.write),
	validate("json", reorderCategoriesSchema),
	async (c) => {
...
adminCatalogRoutes.get(
	...
	async (c) => c.json(await categories.getCategory(c.req.valid("param").id)),
);
...
adminCatalogRoutes.get(
	...
	async (c) => c.json(await products.listProducts(c.req.valid("query"))),
);
...
adminCatalogRoutes.post(
	"/products/import",
	requirePermission(PERMISSIONS.catalog.import),
	validate("json", z.object({ csv: z.string().min(1).max(5_000_000), dryRun: z.boolean().default(false) })),
	async (c) => {
...
adminCatalogRoutes.get(
	...
	requirePermission(PERMISSIONS.catalog.read),
	validate("param", idParam),
	async (c) => c.json(await products.getProduct(c.req.valid("param").id)),
);
...
adminCatalogRoutes.post(
	...
	requirePermission(PERMISSIONS.catalog.write),
	validate("param", idParam),
	validate("json", duplicateProductSchema),
	async (c) => {
		const { id } = c.req.valid("param");
		const created = await products.duplicateProduct(
			id,
			c.req.valid("json"),
			currentUser(c).sub,
		);
...
adminCatalogRoutes.post(
	...
	requirePermission(PERMISSIONS.catalog.write),
	validate("param", idParam),
	validate("json", variantInputSchema),
	async (c) => {
		const { id } = c.req.valid("param");
...
adminCatalogRoutes.post(
	...
	requirePermission(PERMISSIONS.catalog.write),
	validate("param", idParam),
	validate("json", sizeInputSchema.extend({ variantId: z.uuid().nullish() })),
	async (c) => {
		const { id } = c.req.valid("param");
