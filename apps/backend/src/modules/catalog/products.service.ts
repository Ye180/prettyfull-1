...
): number => sizeOverride ?? variantOverride ?? basePrice;
...
const stockFacts = (stock: RawStock | undefined) => {
 ...
 */
const hydrateProducts = async (productIds: string[]): Promise<Map<string, Product>> => {
	if (productIds.length === 0) return new Map();

	const [productRows, imageRows, variantRows, variantImageRows, sizeRows, categoryRows, stockRows] =
		await Promise.all([
			...
			db
				...
				.innerJoin(
					t.productVariants,
					eq(t.productVariants.id, t.variantImages.variantId),
				)
				.where(inArray(t.productVariants.productId, productIds))
			...
			db
				.select({
					...
					ownerProductId: sql<string>`coalesce(${t.sizes.productId}, ${t.productVariants.productId})`,
				})
				.from(t.sizes)
				.leftJoin(t.productVariants, eq(t.productVariants.id, t.sizes.variantId))
				.where(
					sql`coalesce(${t.sizes.productId}, ${t.productVariants.productId}) in ${productIds}`,
				)
	...
	for (const product of productRows) {
		const variants: Variant[] = variantRows
			...
			.map((variant) => {
				const variantSizes: Size[] = sizeRows
					.filter((size) => size.variantId === variant.id)
					.map((size) => {
						const facts = stockFacts(
							stockByScope.get(stockKey(product.id, variant.id, size.id)),
						);
						return {
							id: size.id,
							productId: null,
				...
				return {
					...
					stockStatus: deriveStockStatus(
						aggregated.availableQuantity,
						product.lowStockThreshold,
					),
				} satisfies Variant;
			});

		const productSizes: Size[] = sizeRows
...
export const getProductBySlug = async (
	...
	const filters: SQL[] = [eq(t.products.slug, slug), isNull(t.products.deletedAt)];
	if (options.publishedOnly) filters.push(eq(t.products.status, "published"));
...
const resolveCategoryIds = async (
	query: Pick<ProductListQuery, "categoryId" | "categorySlug">,
): Promise<string[] | null> => {
	...
	if (query.categoryId) return getDescendantIds(query.categoryId);

	return null;
};

const categoryFilter = (categoryIds: string[] | null): SQL | null => {
...
const searchFilter = (q: string | undefined): SQL | null =>
	q
		? sql`
...
export const listProducts = async (
	...
	if (!query.includeArchived) filters.push(isNull(t.products.deletedAt));
	if (query.status) filters.push(eq(t.products.status, query.status));
	if (query.kind) filters.push(eq(t.products.kind, query.kind));
	if (query.isFeatured !== undefined) filters.push(eq(t.products.isFeatured, query.isFeatured));
	if (query.minPrice !== undefined) filters.push(gte(t.products.basePrice, query.minPrice));
	if (query.maxPrice !== undefined) filters.push(lte(t.products.basePrice, query.maxPrice));
	if (query.tag) filters.push(sql`${query.tag} = any(${t.products.tags})`);
	if (query.onSale) {
		filters.push(
			sql`${t.products.compareAtPrice} is not null and ${t.products.compareAtPrice} > ${t.products.basePrice}`,
		);
	}
	...
	if (query.stockStatus) {
		...
		if (query.stockStatus === "out_of_stock") filters.push(sql`${available} <= 0`);
		else if (query.stockStatus === "low_stock") {
			filters.push(sql`${available} > 0 and ${available} <= ${t.products.lowStockThreshold}`);
		} else filters.push(sql`${available} > ${t.products.lowStockThreshold}`);
	}

	const where = filters.length > 0 ? and(...filters) : undefined;
	const { limit, offset } = toSqlPagination(query);

	const sortColumn = SORTABLE[query.sort as keyof typeof SORTABLE] ?? t.products.createdAt;
 ...
 * impossible.
 */
export const getProductFacets = async (query: ProductFacetsQuery): Promise<ProductFacets> => {
	const filters: SQL[] = [isNull(t.products.deletedAt), eq(t.products.status, "published")];
	...
	const [priceRow] = await db
		.select({
			min: sql<number>`coalesce(min(${t.products.basePrice}), 0)`,
			max: sql<number>`coalesce(max(${t.products.basePrice}), 0)`,
		})
		.from(t.products)
		.where(where);
	...
	const sizeRows = await db
		.select({
			label: t.sizes.label,
			position: sql<number>`min(${t.sizes.position})`,
		})
		.from(t.sizes)
		.leftJoin(t.productVariants, eq(t.productVariants.id, t.sizes.variantId))
		.where(
			and(
				eq(t.sizes.status, "active"),
				sql`coalesce(${t.sizes.productId}, ${t.productVariants.productId}) in ${productIds}`,
			),
		)
	...
	const colorRows = await db
		.select({
			...
			position: sql<number>`min(${t.productVariants.position})`,
		})
		.from(t.productVariants)
		.where(
			and(
				eq(t.productVariants.status, "active"),
				inArray(t.productVariants.productId, productIds),
			),
		)
...
const assertSlugAvailable = async (slug: string, excludeId?: string): Promise<void> => {
	const filters: SQL[] = [eq(t.products.slug, slug), isNull(t.products.deletedAt)];
	if (excludeId) filters.push(ne(t.products.id, excludeId));
...
const createInventory = async (
	tx: Transaction,
	scope: { productId: string; variantId?: string | null; sizeId?: string | null },
	quantity: number,
	lowStockThreshold: number,
): Promise<void> => {
	const [item] = await tx
...
export const createProduct = async (
	...
	if (input.kind === "variant" && input.sizes.length > 0) {
		throw invalidProductModel(
			"Un produit à variantes porte ses tailles au niveau des variantes.",
		);
	}
	...
	const productId = await db.transaction(async (tx) => {
		...
		if (input.images.length > 0) {
			await tx.insert(t.productImages).values(
		...
		if (input.kind === "variant") {
				...
				if (variant.sizes.length > 0) {
					for (const [sizeIndex, size] of variant.sizes.entries()) {
						...
						await createInventory(
							tx,
							{ productId: id, variantId, sizeId: insertedSize!.id },
			...
			for (const [sizeIndex, size] of input.sizes.entries()) {
				...
				await createInventory(
					tx,
					{ productId: id, sizeId: insertedSize!.id },
					size.initialQuantity ?? 0,
					size.lowStockThreshold ?? input.lowStockThreshold,
				);
			}
...
export const archiveProduct = async (id: string): Promise<void> => {
	...
	if (!updated) throw notFound("Produit");
};
...
export const duplicateProduct = async (
	id: string,
	options: { name?: string; slug?: string; includeImages: boolean },
	createdBy: string,
): Promise<Product> => {
