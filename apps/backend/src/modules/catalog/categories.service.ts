...
const toCategory = (row: Awaited<ReturnType<typeof fetchRows>>[number]): Category => ({
	id: row.id,
...
const fetchRows = (where: SQL | undefined) =>
	db
		.select(selection)
		.from(t.categories)
		.where(where)
...
const assertSlugAvailable = async (slug: string, excludeId?: string): Promise<void> => {
	const filters: SQL[] = [eq(t.categories.slug, slug), isNull(t.categories.deletedAt)];
	if (excludeId) filters.push(ne(t.categories.id, excludeId));
...
const resolvePlacement = async (
	...
	if (!parent) throw badRequest("Catégorie parente introuvable.", { parentId: ["Inconnue."] });

	if (parent.depth + 1 > MAX_DEPTH) {
		throw badRequest(
			`Profondeur maximale atteinte (${MAX_DEPTH} niveaux).`,
			{ parentId: ["Arborescence trop profonde."] },
		);
	}
...
export const listCategories = async (query: {
	...
	const filters: SQL[] = [isNull(t.categories.deletedAt)];

	if (query.parentId) filters.push(eq(t.categories.parentId, query.parentId));
	if (query.status) filters.push(eq(t.categories.status, query.status));
	if (query.isFeatured !== undefined) {
		filters.push(eq(t.categories.isFeatured, query.isFeatured));
	}
	if (query.q) filters.push(ilike(t.categories.name, `%${query.q}%`));
...
export const getCategoryTree = async (
	...
	const filters: SQL[] = [isNull(t.categories.deletedAt)];
	if (options.activeOnly) filters.push(eq(t.categories.status, "active"));
...
export const getCategoryBySlug = async (slug: string): Promise<Category> => {
	const [row] = await fetchRows(
		and(eq(t.categories.slug, slug), isNull(t.categories.deletedAt)),
	).limit(1);
...
export const getCategory = async (id: string): Promise<Category> => {
	const [row] = await fetchRows(
		and(eq(t.categories.id, id), isNull(t.categories.deletedAt)),
	).limit(1);
 ...
 */
const rewriteSubtreePaths = async (oldPath: string, newPath: string): Promise<void> => {
	...
	if (input.parentId !== undefined && input.parentId !== current.parentId) {
		...
		if (input.parentId) {
			...
			if (target && (target.path === current.path || target.path.startsWith(`${current.path}/`))) {
				throw badRequest(
					"Impossible de déplacer une catégorie sous l'une de ses descendantes.",
					{ parentId: ["Référence circulaire."] },
				);
			}
		}
...
export const archiveCategory = async (id: string): Promise<void> => {
	...
	if ((children?.total ?? 0) > 0) {
		throw conflict(
			`Cette catégorie contient ${children!.total} sous-catégorie(s). Déplacez-les d'abord.`,
		);
	}
	...
	if ((products?.total ?? 0) > 0) {
		throw conflict(
			`Cette catégorie contient ${products!.total} produit(s). Déplacez-les ou archivez-les d'abord.`,
		);
	}
...
export const countProductsByCategory = async (): Promise<Map<string, number>> => {
	const rows = await db
		.select({
