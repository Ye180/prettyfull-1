 ...
 * réciproquement.
 */
const buildAuthRoutes = (kind: UserKind, scope: CookieScope) => {
	const app = new Hono<AppEnv>();

	const context = (c: { get: (key: "userAgent" | "clientIp") => string | null }) => ({
		userAgent: c.get("userAgent"),
		ipAddress: c.get("clientIp"),
	});

	app.post("/login", validate("json", loginSchema), async (c) => {
		const { auth, refreshToken } = await service.login(
			c.req.valid("json"),
			kind,
			context(c),
		);
	...
	app.post("/refresh", validate("json", refreshSchema), async (c) => {
		...
		if (!token) throw unauthorized("Aucune session à renouveler.");
	...
	app.get("/me", requireAuth, requireKind(kind), async (c) =>
		c.json(await service.getProfile(currentUser(c).sub)),
	);
	...
	app.patch(
		...
		async (c) => c.json(await service.updateProfile(currentUser(c).sub, c.req.valid("json"))),
	);
