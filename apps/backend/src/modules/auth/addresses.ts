import type { Address, AddressInput } from "@prettyfull/contracts";
...
const toAddress = (row: typeof t.addresses.$inferSelect): Address => ({
	id: row.id,
...
const clearDefaults = async (
	userId: string,
	input: Pick<AddressInput, "isDefaultShipping" | "isDefaultBilling">,
	exceptId?: string,
): Promise<void> => {
	if (!input.isDefaultShipping && !input.isDefaultBilling) return;
...
export const storeAddressRoutes = new Hono<AppEnv>();
...
storeAddressRoutes.post(
	"/addresses",
	validate("json", addressInputSchema),
	async (c) => {
...
storeAddressRoutes.patch(
	"/addresses/:id",
	validate("param", idParam),
	validate("json", addressInputSchema.partial()),
	async (c) => {
		const userId = currentUser(c).sub;
		...
		if (!updated) throw notFound("Adresse");
...
storeAddressRoutes.delete("/addresses/:id", validate("param", idParam), async (c) => {
	const [deleted] = await db
		...
		.where(
			and(
				eq(t.addresses.id, c.req.valid("param").id),
				eq(t.addresses.userId, currentUser(c).sub),
			),
		)
		.returning({ id: t.addresses.id });

	if (!deleted) throw notFound("Adresse");
