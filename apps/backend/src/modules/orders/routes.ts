...
adminOrdersRoutes.get(
	...
	async (c) => c.json(await service.listOrders(c.req.valid("query"))),
);
...
adminOrdersRoutes.get(
	...
	async (c) => c.json(await service.getOrder(c.req.valid("param").id)),
);
...
adminOrdersRoutes.patch(
	...
	requirePermission(PERMISSIONS.orders.write),
	validate("param", idParam),
	validate("json", updateFulfillmentSchema),
	async (c) => {
		const { id } = c.req.valid("param");
...
adminOrdersRoutes.post(
	"/orders/:id/refund",
	requirePermission(PERMISSIONS.orders.refund),
	validate("param", idParam),
	validate("json", refundOrderSchema),
	async (c) => {
		const { id } = c.req.valid("param");
		const input = c.req.valid("json");
		const user = currentUser(c);
...
adminOrdersRoutes.post(
	...
	requirePermission(PERMISSIONS.orders.write),
	validate("param", idParam),
	validate("json", cancelOrderSchema),
	async (c) => {
		const { id } = c.req.valid("param");
...
adminOrdersRoutes.post(
	...
	requirePermission(PERMISSIONS.orders.write),
	validate("param", idParam),
	validate("json", z.object({ comment: z.string().max(500).optional() })),
	async (c) => {
		const { id } = c.req.valid("param");
		const user = currentUser(c);
 ...
 * à lui seul.
 */
export const storeOrderConfirmationRoutes = new Hono<AppEnv>();

storeOrderConfirmationRoutes.get(
	"/orders/:id/confirmation",
	validate("param", idParam),
	validate("query", z.object({ token: z.string().min(16).max(64) })),
	async (c) =>
		c.json(
			await service.getOrderByToken(
...
storeOrdersRoutes.get("/orders/:id", validate("param", idParam), async (c) =>
	c.json(await service.getOrderForUser(c.req.valid("param").id, currentUser(c).sub)),
);
...
storeOrdersRoutes.post(
	...
	async (c) => {
		...
		return c.json(
			await service.updateOrderStatus(id, "cancelled", {
