...
export class ProductsService {
	 ...
	 */
	// --- CORRECTION ---
	// Cette fonction a été rendue "défensive" pour éviter les crashs
	// si des champs (comme 'name' ou 'price') sont manquants dans la BDD.
	private transformProduct(product: any, language: string): TransformedProduct {
		const lang = language || "fr"; // Assurer un fallback pour la langue

		return {
			id: product._id,
			name: product.name?.[lang] || "", // Ajout d'un fallback
			description: product.description?.[lang] || "",
			category: product.category || undefined,
			link: product.link,
			variable: product.variable || [],
			notVariable: product.notVariable || undefined,
			smallDescription: product.smallDescription?.[lang] || "", // Ajout d'un fallback
			slug: product.slug,
			sku: product.sku,
			price: {
				// Utilisation de ?. pour éviter les crashs si 'price' ou 'amount' est null
				amount: product.price?.amount?.[lang] || 0,
				currency: product.price?.currency?.[lang] || "XOF", // Ajout d'un fallback
			},
			solde: product.solde || false,
			promotion: product.promotion
				? {
						reduced_price: {
							// Utilisation de ?. partout
							amount:
								product.promotion?.reduced_price?.[lang]?.amount || 0,
							currency:
								product.promotion?.reduced_price?.[lang]?.currency || "XOF",
						},
						pourcentage: product.promotion?.pourcentage || 0,
				  }
				: undefined,
			isLoading: product.isLoading || false,
			label: product.label?.[lang] || "", // Ajout d'un fallback
