...
export class WishlistsService {
  ...
  async addToWishlist(userId: string, productId: string): Promise<any> {
    ...
    try {
      ...
      return {
        message: added === 1 ? 'Produit ajouté à la wishlist' : 'Produit déjà dans la wishlist',
        added: added === 1,
      };
    } catch (error) {
  ...
  async getWishlist(userId: string): Promise<any> {
    ...
    try {
      ...
      throw new BadRequestException('Erreur lors de la récupération de la wishlist');
    }
  }
  ...
  async removeFromWishlist(userId: string, productId: string): Promise<any> {
    ...
    try {
      ...
      return {
        message: removed === 1 ? 'Produit supprimé de la wishlist' : 'Produit non trouvé dans la wishlist',
        removed: removed === 1,
      };
    } catch (error) {
      throw new BadRequestException('Erreur lors de la suppression de la wishlist');
    }
  }
  ...
  async isInWishlist(userId: string, productId: string): Promise<boolean> {
    ...
    try {
      ...
      throw new BadRequestException('Erreur lors de la vérification de la wishlist');
    }
  }
}
