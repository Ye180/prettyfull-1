...
/**
 * Gestion des comptes back-office et des fiches clientes (§2.7, §4.5, §4.8).
 */
...
interface ListQuery {
	...
	limit: number;
	q?: string;
	kind?: (typeof t.userKindEnum.enumValues)[number];
 ...
 */
const loadRolesFor = async (userIds: string[]): Promise<Map<string, string[]>> => {
...
const toUser = (row: UserRow, roles: string[]): User => ({
	id: row.id,
	email: row.email,
...
export const listUsers = async (query: ListQuery): Promise<Paginated<User>> => {
	const filters: SQL[] = [isNull(t.users.deletedAt)];

	if (query.kind) filters.push(eq(t.users.kind, query.kind));
	if (query.status) filters.push(eq(t.users.status, query.status));

	if (query.q) {
		const pattern = `%${query.q}%`;
		const search = or(
			ilike(t.users.email, pattern),
			ilike(t.users.firstName, pattern),
			ilike(t.users.lastName, pattern),
		);
	...
	return paginate(
		rows.map((row) => toUser(row, rolesByUser.get(row.id) ?? [])),
		query,
		totals?.total ?? 0,
	);
};
...
const setRoles = async (userId: string, roleKeys: RoleKey[]): Promise<void> => {
	const rows = await db
...
export const updateStaff = async (
	...
	if (target.kind !== "staff") throw badRequest("Ce compte n'est pas un compte back-office.");
	...
	if (roles) await setRoles(userId, roles);
