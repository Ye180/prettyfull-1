...
import {
	...
	isConfigured,
	listPaymentAdapters,
} from "../../integrations/payment/registry.js";
...
const toProviderConfig = (
	row: {
		...
		key: string;
		isEnabled: boolean;
		environment: (typeof t.integrationEnvironmentEnum.enumValues)[number];
...
export const listPaymentProviders = async (): Promise<ProviderConfig[]> => {
	const rows = await db
		.select()
...
export const listShippingProviders = async (): Promise<ProviderConfig[]> => {
	const rows = await db
		.select()
	...
	return listShippingAdapters().map((adapter) =>
		toProviderConfig(
			known.get(adapter.key) ?? {
				id: "00000000-0000-0000-0000-000000000000",
				key: adapter.key,
				isEnabled: false,
				environment: "test" as const,
...
export const updatePaymentProvider = async (
	...
	if (!adapter) throw notFound("Agrégateur de paiement");
	...
	if (input.credentials) {
		for (const [name, value] of Object.entries(input.credentials)) {
			if (value === "") delete mergedCredentials[name];
			else Object.assign(mergedCredentials, encryptCredentials({ [name]: value }));
		}
	}
	...
	if (willEnable && !isConfigured(adapter, mergedCredentials)) {
		...
		throw conflict(
			`Impossible d'activer « ${adapter.name} » : ${missing.join(", ")} manquant(s).`,
			{ credentials: missing },
		);
	}
	...
	const values = {
		...
		environment: input.environment ?? existing?.environment ?? ("test" as const),
...
export const updateShippingProvider = async (
	...
	if (!adapter) throw notFound("Agrégateur de livraison");
...
export const testPaymentProvider = async (key: string) => {
	const adapter = getPaymentAdapter(key);
	if (!adapter) throw notFound("Agrégateur de paiement");
...
export const listAvailablePaymentOptions = async (): Promise<PaymentOption[]> => {
	...
	return rows.flatMap((row) => {
		...
		if (!adapter || !isConfigured(adapter, row.credentials)) return [];
...
export const listShippingZones = async (): Promise<(ShippingZone & { rates: ShippingRate[] })[]> => {
...
export const updateShippingZone = async (
	id: string,
	input: Partial<{ name: string; countryCodes: string[]; isActive: boolean }>,
): Promise<ShippingZone> => {
	...
	if (!updated) throw notFound("Zone de livraison");
...
export const deleteShippingZone = async (id: string): Promise<void> => {
	...
	if (!deleted) throw notFound("Zone de livraison");
};

export const createShippingRate = async (input: ShippingRateInput): Promise<ShippingRate> => {
	...
	return { ...created!, zoneName: undefined } as ShippingRate;
};
...
export const updateShippingRate = async (
	...
	if (!updated) throw notFound("Tarif de livraison");
...
export const deleteShippingRate = async (id: string): Promise<void> => {
	...
	if (!deleted) throw notFound("Tarif de livraison");
};
...
export const listTransactions = async (query: {
	...
	orderId?: string;
	providerKey?: string;
	kind?: (typeof t.transactionKindEnum.enumValues)[number];
	status?: (typeof t.transactionStatusEnum.enumValues)[number];
	from?: string;
	to?: string;
}): Promise<Paginated<Transaction & { orderDisplayId: number }>> => {
	const filters: SQL[] = [];

	if (query.orderId) filters.push(eq(t.transactions.orderId, query.orderId));
	if (query.providerKey) filters.push(eq(t.transactions.providerKey, query.providerKey));
	if (query.kind) filters.push(eq(t.transactions.kind, query.kind));
	if (query.status) filters.push(eq(t.transactions.status, query.status));
	if (query.from) filters.push(gte(t.transactions.createdAt, new Date(query.from)));
	if (query.to) filters.push(lte(t.transactions.createdAt, new Date(query.to)));
