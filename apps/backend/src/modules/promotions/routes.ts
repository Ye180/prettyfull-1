...
export const adminPromotionsRoutes = new Hono<AppEnv>();

adminPromotionsRoutes.get(
	"/promo-codes",
	requirePermission(PERMISSIONS.promotions.read),
	validate("query", promoCodeListQuerySchema),
	async (c) => c.json(await service.listPromoCodes(c.req.valid("query"))),
);
...
adminPromotionsRoutes.get(
	...
	async (c) => c.json(await service.getPromoCode(c.req.valid("param").id)),
);

adminPromotionsRoutes.post(
	"/promo-codes",
	requirePermission(PERMISSIONS.promotions.write),
	validate("json", promoCodeInputSchema),
	async (c) => {
...
adminPromotionsRoutes.patch(
	"/promo-codes/:id",
	requirePermission(PERMISSIONS.promotions.write),
	validate("param", idParam),
	validate("json", updatePromoCodeSchema),
	async (c) => {
