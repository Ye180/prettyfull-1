...
export const inventoryItems = pgTable(
	...
	(table) => [
		...
		index("inventory_items_low_stock_idx").on(
			sql`(${table.quantity} - ${table.reservedQuantity})`,
		),
		check(
			"inventory_items_reserved_positive",
			sql`${table.reservedQuantity} >= 0`,
		),
		// Sans découvert autorisé, on ne peut pas réserver plus que le stock.
		check(
			"inventory_items_reserved_within_stock",
			sql`${table.allowBackorder} = true or ${table.reservedQuantity} <= ${table.quantity}`,
		),
	],
);
/**
...
export const stockMovements = pgTable(
	...
	(table) => [
		index("stock_movements_item_idx").on(
