...
import { activationStatusEnum, contentStatusEnum, currencyEnum, productKindEnum, } from "./enums.js";
...
export const products = pgTable("products", {
    ...
    index("products_search_idx").using("gin", sql `to_tsvector('french', pf_unaccent(coalesce(${table.name}, '') || ' ' || coalesce(${table.shortDescription}, '')))`),
    index("products_tags_idx").using("gin", table.tags),
    check("products_compare_at_price_check", sql `${table.compareAtPrice} is null or ${table.compareAtPrice} > ${table.basePrice}`),
]);
...
export const variantImages = pgTable("variant_images", {
    ...
    position: integer("position").notNull().default(0),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [index("variant_images_variant_idx").on(table.variantId, table.position)]);
/**
...
export const sizes = pgTable("sizes", {
    ...
    check("sizes_owner_xor", sql `(${table.productId} is not null) <> (${table.variantId} is not null)`),
    ...
    uniqueIndex("sizes_sku_unique").on(table.sku).where(sql `${table.sku} is not null`),
]);
...
export const sizesRelations = relations(sizes, ({ one }) => ({
    product: one(products, { fields: [sizes.productId], references: [products.id] }),
    variant: one(productVariants, {
