...
import { cartStatusEnum, currencyEnum, fulfillmentStatusEnum, orderStatusEnum, paymentStatusEnum, } from "./enums.js";
...
export const carts = pgTable("carts", {
    ...
    uniqueIndex("carts_active_session_unique")
        ...
        .where(sql `${table.status} = 'active' and ${table.sessionToken} is not null`),
...
export const cartItems = pgTable("cart_items", {
    ...
    uniqueIndex("cart_items_line_unique").on(table.cartId, table.inventoryItemId),
...
export const orders = pgTable("orders", {
    id: uuid("id").primaryKey().defaultRandom(),
    displayId: integer("display_id").notNull().unique().generatedByDefaultAsIdentity({
        startWith: 1000,
    }),
    ...
    check("orders_refunded_within_total", sql `${table.refundedTotal} >= 0 and ${table.refundedTotal} <= ${table.total}`),
]);
/**
...
export const orderItems = pgTable("order_items", {
    ...
    check("order_items_refunded_within_quantity", sql `${table.refundedQuantity} >= 0 and ${table.refundedQuantity} <= ${table.quantity}`),
]);
