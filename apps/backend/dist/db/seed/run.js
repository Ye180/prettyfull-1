...
const seedUsers = async (roleIdByKey) => {
	...
	const staffSeeds = [
		{
			email: "admin@prettyfull.shop",
			firstName: "Awa",
			lastName: "Koné",
			role: "super_admin",
		},
		{
			email: "catalogue@prettyfull.shop",
			firstName: "Fatou",
			lastName: "Diallo",
			role: "catalog_manager",
		},
		{
			email: "commandes@prettyfull.shop",
			firstName: "Ibrahim",
			lastName: "Traoré",
			role: "order_manager",
		},
		{
			email: "support@prettyfull.shop",
			firstName: "Mariam",
			lastName: "Bamba",
			role: "support",
		},
...
const seedSettingsAndIntegrations = async (adminId) => {
	...
	await db.insert(t.paymentProviders).values([
		...
		},
		{
			...
			description:
				"Encaissement à la remise du colis, sans prestataire externe.",
			isEnabled: true,
			environment: "live",
			position: 1,
		},
	...
	const zones = await db
		...
		.values([
			{ name: "Abidjan", countryCodes: ["ci"], isActive: true },
			{
				...
				countryCodes: ["sn", "ml", "bf", "tg", "bj"],
				isActive: true,
			},
			{
				name: "International",
				countryCodes: ["fr", "be", "us"],
				isActive: true,
			},
		])
	...
	console.log(
		"  paramètres, taxe, 2 moyens de paiement, 3 zones et 4 tarifs de port",
	);
};
// --- Catalogue -------------------------------------------------------------
const seedCatalog = async (adminId) => {
	...
	const stockKey = (product, variant, size) =>
	...
	for (const seed of SEED_PRODUCTS) {
		...
		await db
			.insert(t.productImages)
			.values(
				seed.images.map((url, position) => ({
					productId,
					url,
					alt: seed.name,
					position,
		...
		if (seed.kind === "variant") {
			...
			for (const size of seed.sizes) {
				const row = sizeRows.find(
					(candidate) => candidate.label === size.label,
				);
				inventorySeeds.push({
					productId,
					variantId: null,
