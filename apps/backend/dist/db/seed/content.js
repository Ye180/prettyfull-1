...
export const seedContent = async (categoryIdBySlug, productIdBySlug) => {
    ...
    const categoryEntries = SEED_CATEGORIES.filter((category) => category.sectionKey).flatMap((category, index) => {
    ...
    console.log(`  contenu : 3 bannières, 3 pages statiques, ${categoryEntries.length + productEntries.length} mises en avant`);
};
