 ...
 */
export const internalShippingAdapter = {
    key: "internal",
    ...
    quote: async (context) => {
        ...
        const rows = await db
            ...
            .where(and(eq(t.shippingRates.isActive, true), eq(t.shippingZones.isActive, true), eq(t.shippingRates.providerKey, "internal"), sql `${context.destination.countryCode} = any(${t.shippingZones.countryCodes})`))
        ...
        return rows.flatMap((rate) => {
            ...
            if (rate.kind === "weight") {
                ...
                if (context.weightGrams < min || context.weightGrams > max)
                    return [];
            }
            const free = rate.freeAboveTotal !== null && context.subtotal >= rate.freeAboveTotal;
            return [
                {
                    rateId: rate.id,
