 ...
 */
export const storeCartRoutes = new Hono();
...
storeCartRoutes.get("/cart", async (c) => c.json(await service.getCart(await resolveCartId(c))));
...
storeCartRoutes.patch("/cart/items/:itemId", validate("param", itemParam), validate("json", updateCartItemSchema), async (c) => {
    const cartId = await resolveCartId(c);
    await service.updateItemQuantity(cartId, c.req.valid("param").itemId, c.req.valid("json").quantity);
    return c.json(await service.getCart(cartId));
});
storeCartRoutes.delete("/cart/items/:itemId", validate("param", itemParam), async (c) => {
    const cartId = await resolveCartId(c);
...
storeCartRoutes.put("/cart/addresses", validate("json", z.object({
...
storeCartRoutes.get("/cart/shipping-options", async (c) => c.json(await service.listShippingOptions(await resolveCartId(c))));
