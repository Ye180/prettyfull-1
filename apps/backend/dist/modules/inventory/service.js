 ...
 */
const lockInventoryItem = async (tx, inventoryItemId) => {
 ...
 */
export const applyMovement = async (tx, params) => {
    if (params.delta === 0) {
    ...
    if (quantityAfter < 0) {
        throw insufficientStock(`Stock insuffisant : ${quantityBefore} en stock, ${Math.abs(params.delta)} demandés.`, { available: [String(quantityBefore)] });
    }
    ...
    if (!item.allowBackorder && quantityAfter < item.reservedQuantity) {
        throw insufficientStock(`Stock insuffisant : ${item.reservedQuantity} unités sont réservées par des paiements en cours.`, { reserved: [String(item.reservedQuantity)] });
    }
    await tx
 ...
 */
export const adjustStock = async (params) => db.transaction((tx) => applyMovement(tx, {
    inventoryItemId: params.inventoryItemId,
    delta: params.delta,
    reason: params.reason,
    note: params.note ?? null,
...
export const setStock = async (params) => db.transaction(async (tx) => {
    const item = await lockInventoryItem(tx, params.inventoryItemId);
 ...
 */
export const reserveStock = async (tx, requests, context) => {
    const ordered = [...requests].sort((a, b) => a.inventoryItemId.localeCompare(b.inventoryItemId));
    for (const request of ordered) {
        ...
        if (!item.allowBackorder && available < request.quantity) {
            throw insufficientStock(available <= 0
                ...
                : `« ${request.label} » : ${available} unité(s) restante(s), ${request.quantity} demandée(s).`, { [request.inventoryItemId]: [String(Math.max(0, available))] });
        }
        await tx
            .update(t.inventoryItems)
            .set({
 ...
 */
export const consumeReservations = async (tx, orderId) => {
    const reservations = await tx
        ...
        .where(and(eq(t.stockReservations.orderId, orderId), eq(t.stockReservations.status, "active")));
 ...
 * bougé.
 */
export const releaseReservations = async (tx, target, status = "released") => {
...
export const expireStaleReservations = async () => {
    const expired = await db
        ...
        .from(t.stockReservations)
        .where(and(eq(t.stockReservations.status, "active"), lte(t.stockReservations.expiresAt, new Date())))
        .limit(500);
    if (expired.length === 0)
        return 0;
 ...
 */
export const findInventoryItem = async (scope, executor = db) => {
    const rows = await executor
        ...
        .from(t.inventoryItems)
        .where(and(eq(t.inventoryItems.productId, scope.productId), sql `${t.inventoryItems.variantId} is not distinct from ${scope.variantId ?? null}`, sql `${t.inventoryItems.sizeId} is not distinct from ${scope.sizeId ?? null}`))
        .limit(1);
    return rows[0] ?? null;
};
/** Crée le point de stock d'un article s'il n'existe pas encore. */
export const ensureInventoryItem = async (tx, scope, options = {}) => {
...
export const updateInventorySettings = async (inventoryItemId, input) => {
...
// --- Lectures --------------------------------------------------------------
const availableExpression = sql `${t.inventoryItems.quantity} - ${t.inventoryItems.reservedQuantity}`;
...
const toInventoryRow = (row) => {
    ...
    return {
        ...
        stockStatus: deriveStockStatus(availableQuantity, row.lowStockThreshold, row.allowBackorder),
...
export const listInventory = async (query) => {
    const filters = [isNull(t.products.deletedAt)];
    if (query.productId)
        filters.push(eq(t.inventoryItems.productId, query.productId));
    if (query.q) {
        const pattern = `%${query.q}%`;
        const search = or(ilike(t.products.name, pattern), ilike(t.products.sku, pattern), ilike(t.productVariants.sku, pattern), ilike(t.sizes.sku, pattern));
        if (search)
            filters.push(search);
    }
    ...
    if (query.stockStatus === "out_of_stock") {
        ...
        filters.push(sql `${availableExpression} > 0 and ${availableExpression} <= ${t.inventoryItems.lowStockThreshold}`);
    }
...
export const listMovements = async (query) => {
    ...
    if (query.productId)
        filters.push(eq(t.inventoryItems.productId, query.productId));
    if (query.orderId)
        filters.push(eq(t.stockMovements.orderId, query.orderId));
    if (query.userId)
        filters.push(eq(t.stockMovements.userId, query.userId));
    if (query.reason)
        filters.push(eq(t.stockMovements.reason, query.reason));
    if (query.from)
        filters.push(gte(t.stockMovements.createdAt, new Date(query.from)));
    if (query.to)
        filters.push(lte(t.stockMovements.createdAt, new Date(query.to)));
...
export const listLowStockAlerts = async (limit = 20) => {
    const rows = await db
        ...
        .leftJoin(t.productVariants, eq(t.productVariants.id, t.inventoryItems.variantId))
        .leftJoin(t.sizes, eq(t.sizes.id, t.inventoryItems.sizeId))
        .where(and(isNull(t.products.deletedAt), eq(t.products.status, "published"), sql `${availableExpression} <= ${t.inventoryItems.lowStockThreshold}`))
        .orderBy(sql `${availableExpression} asc`)
        .limit(limit);
    return rows;
};
/** Agrège le stock disponible de plusieurs produits, en une requête. */
export const summarizeByProduct = async (productIds) => {
    if (productIds.length === 0)
        return new Map();
    const rows = await db
        .select({
        productId: t.inventoryItems.productId,
        quantity: sql `sum(${t.inventoryItems.quantity})::int`,
        available: sql `sum(${availableExpression})::int`,
        threshold: sql `max(${t.inventoryItems.lowStockThreshold})::int`,
    })
        .from(t.inventoryItems)
