import { DEFAULT_STORE_SETTINGS, storeSettingsSchema, } from "@prettyfull/contracts";
...
export const updateTaxRate = async (id, input) => {
    return db.transaction(async (tx) => {
        if (input.isDefault) {
            await tx.update(t.taxRates).set({ isDefault: false }).where(ne(t.taxRates.id, id));
        }
