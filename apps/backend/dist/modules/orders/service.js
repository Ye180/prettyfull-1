	...
	for (const order of orderRows) {
		result.set(order.id, {
			...
			billingAddress: order.billingAddress
				? toAddress(order.billingAddress)
				: null,
...
const loadProviderConfig = async (providerKey) => {
	...
	if (!adapter) {
		throw paymentError(
			"Aucun adaptateur n'est enregistré pour ce moyen de paiement.",
		);
	}
	return {
		adapter,
 ...
 */
export const confirmationTokenFor = (orderId) =>
...
export const checkout = async (cartId, input, actor) => {
	...
	const shipping = await cart.resolveShippingAmount(
		cartId,
	...
	const { adapter, runtime } = await loadProviderConfig(
		input.paymentProviderKey,
	);
	const expiresAt = new Date(
		Date.now() + settings.stockReservationMinutes * 60_000,
	);
	...
	if (initiation.status === "succeeded") {
		await markOrderPaid(orderId, {
			comment: `Paiement ${adapter.name} confirmé`,
		});
	}
...
export const markPaymentFailed = async (orderId, reason) => {
	await db
		...
		.where(
			and(eq(t.orders.id, orderId), eq(t.orders.paymentStatus, "pending")),
		);
	await db.transaction(async (tx) => {
		await appendStatusHistory(
			tx,
	...
	const amount = input.items
		? input.items.reduce((total, entry) => {
				const item = order.items.find(
					(candidate) => candidate.id === entry.orderItemId,
				);
