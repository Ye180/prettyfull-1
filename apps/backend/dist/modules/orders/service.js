import { canTransitionOrder, } from "@prettyfull/contracts";
 ...
 */
// --- Lecture ---------------------------------------------------------------
const toAddress = (value) => value ? value : {};
const hydrateOrders = async (orderIds, options = {}) => {
...
export const getOrderForUser = async (id, userId) => {
...
export const listOrders = async (query) => {
    const filters = [];
    if (query.status)
        filters.push(eq(t.orders.status, query.status));
    if (query.paymentStatus)
        filters.push(eq(t.orders.paymentStatus, query.paymentStatus));
    if (query.fulfillmentStatus) {
        filters.push(eq(t.orders.fulfillmentStatus, query.fulfillmentStatus));
    }
    if (query.userId)
        filters.push(eq(t.orders.userId, query.userId));
    if (query.email)
        filters.push(ilike(t.orders.email, `%${query.email}%`));
    if (query.from)
        filters.push(gte(t.orders.createdAt, new Date(query.from)));
    if (query.to)
        filters.push(lte(t.orders.createdAt, new Date(query.to)));
    if (query.minTotal !== undefined)
        filters.push(gte(t.orders.total, query.minTotal));
    if (query.maxTotal !== undefined)
        filters.push(lte(t.orders.total, query.maxTotal));
    if (query.q) {
        ...
        const search = or(ilike(t.orders.email, `%${query.q}%`), ilike(t.orders.trackingNumber, `%${query.q}%`), ...(Number.isFinite(numeric) && numeric > 0
...
// --- Passage en commande ---------------------------------------------------
const loadProviderConfig = async (providerKey) => {
    const [row] = await db
        .select()
    ...
    if (!row.isEnabled)
        throw badRequest("Ce moyen de paiement n'est pas disponible.");
...
const appendStatusHistory = async (tx, orderId, fromStatus, toStatus, comment, actor) => {
 ...
 */
export const confirmationTokenFor = (orderId) => createHmac("sha256", env.JWT_SECRET)
 ...
 */
export const getOrderByToken = async (id, token) => {
    if (!safeCompare(token, confirmationTokenFor(id)))
        throw notFound("Commande");
    return getOrder(id);
};
/**
 ...
 */
export const checkout = async (cartId, input, actor) => {
    const settings = await getStoreSettings();
    const lines = await cart.loadCartLines(cartId);
    if (lines.length === 0)
        throw badRequest("Votre panier est vide.");
    ...
    if (!shipping)
        throw badRequest("Choisissez un mode de livraison.");
    ...
    if (cartRow.discountCodeId) {
        const [promo] = await db
            ...
            .limit(1);
        if (promo)
            discount = await resolveDiscountForSubtotal(promo.code, subtotal);
    }
    ...
    const { orderId, displayId } = await db.transaction(async (tx) => {
        ...
        const id = order.id;
        await tx.insert(t.orderItems).values(lines.map((line) => ({
            orderId: id,
        ...
        await inventory.reserveStock(tx, lines.map((line) => ({
            inventoryItemId: line.inventoryItemId,
            quantity: line.quantity,
        ...
        if (discount)
            await incrementUsage(discount.promoCodeId, tx);
        await tx
            .update(t.carts)
    ...
    const [transaction] = await db
        ...
        kind: "payment",
        status: initiation.status === "succeeded"
            ? "success"
 ...
 */
export const markOrderPaid = async (orderId, options = {}) => {
    await db.transaction(async (tx) => {
        ...
        await tx
            ...
            .where(and(eq(t.transactions.orderId, orderId), eq(t.transactions.kind, "payment"), eq(t.transactions.status, "pending")));
        await appendStatusHistory(tx, orderId, order.status, "paid", options.comment ?? "Paiement confirmé", options.actor);
    });
};
 ...
 * commande annulée.
 */
export const updateOrderStatus = async (orderId, nextStatus, options) => {
    await db.transaction(async (tx) => {
        const [order] = await tx
        ...
        if (!canTransitionOrder(order.status, nextStatus)) {
            throw conflict(`Transition impossible : une commande « ${order.status} » ne peut pas passer à « ${nextStatus} ».`);
        }
        const now = new Date();
        ...
        await tx
            ...
            ...(nextStatus === "shipped"
                ...
                : {}),
            ...(nextStatus === "preparing" ? { fulfillmentStatus: "preparing" } : {}),
            ...(nextStatus === "delivered"
                ? { fulfillmentStatus: "delivered", deliveredAt: now }
                : {}),
            ...(nextStatus === "cancelled"
                ? { cancelledAt: now, paymentStatus: "cancelled" }
                : {}),
            updatedAt: now,
        })
            .where(eq(t.orders.id, orderId));
        await appendStatusHistory(tx, orderId, order.status, nextStatus, options.comment ?? null, options.actor);
    });
    return getOrder(orderId);
};
export const updateFulfillment = async (orderId, input) => {
 ...
 */
export const refundOrder = async (orderId, input, actor) => {
    const order = await getOrder(orderId);
    ...
    const amount = input.items
        ? input.items.reduce((total, entry) => {
            ...
            if (!item)
                throw badRequest("Ligne de commande introuvable.");
            const remaining = item.quantity - item.refundedQuantity;
            if (entry.quantity > remaining) {
                throw conflict(`« ${item.productName} » : ${remaining} unité(s) remboursable(s), ${entry.quantity} demandée(s).`);
            }
            return total + item.unitPrice * entry.quantity;
        }, 0)
        : (input.amount ?? refundable);
    if (amount <= 0)
        throw badRequest("Le montant à rembourser doit être positif.");
    if (amount > refundable) {
        throw conflict(`Montant remboursable restant : ${refundable}. Demandé : ${amount}.`);
    }
    ...
    if (transaction) {
        ...
        if (adapter?.supportsRefunds && adapter.refund) {
            ...
            if (result.status === "failed") {
                throw paymentError(result.errorMessage ?? "Le prestataire a refusé le remboursement.");
            }
    ...
    await db.transaction(async (tx) => {
        ...
        await tx
            ...
            .where(eq(t.orders.id, orderId));
        if (fullyRefunded) {
            await appendStatusHistory(tx, orderId, order.status, "refunded", input.reason, actor);
        }
    });
    return getOrder(orderId);
};
