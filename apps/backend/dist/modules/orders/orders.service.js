...
let OrdersService = OrdersService_1 = class OrdersService {
    ...
    constructor(orderModel, productModel, notificationsProducer, addressService, userModel, orderEventsService) {
        ...
        try {
            ...
            for (const item of createOrderDto.items) {
                if (!mongoose_2.Types.ObjectId.isValid(item.productId)) {
                    throw new common_1.BadRequestException(`ID de produit invalide: ${item.productId}`);
                }
                const product = await this.productModel.findById(item.productId).exec();
                if (!product) {
                    throw new common_1.NotFoundException(`Produit non trouvé: ${item.productId}`);
                }
                ...
                if (item.selectedVariants && productData.variants) {
                    ...
                    const variantsArray = Array.isArray(productData.variants)
                        ...
                        : [];
                    const variant = variantsArray.find((v) => selectedEntries.every(([key, value]) => {
                        const field = v?.[key];
                ...
                if (stockQuantity < item.quantity) {
                    throw new common_1.BadRequestException(`Stock insuffisant pour le produit ${productData.name?.fr || productData.sku}`);
                }
                ...
                if (item.selectedVariants && productData.variants) {
                    ...
                    await this.productModel.updateOne({ _id: item.productId }, { $inc: { 'notVariable.quantity': -item.quantity } });
                }
            }
            ...
            if (!shippingAddressId) {
                throw new common_1.BadRequestException('Adresse de livraison manquante.');
            }
            ...
            const orderData = {
                ...
                userId: new mongoose_2.Types.ObjectId(createOrderDto.userId),
                items: orderItems,
                ...
                total: totalAmount,
                currency: 'XOF',
                status: orders_schema_1.OrderStatus.PENDING,
                paymentStatus: payment_schema_1.PaymentStatus.PENDING,
            ...
            const savedOrderData = savedOrder;
            const adminEmails = process.env.ADMIN_EMAILS?.split(',') || [];
            if (adminEmails.length > 0) {
                this.orderEventsService.emitNewOrderNotification({
                    orderId: savedOrder._id.toString(),
                    orderNumber: savedOrderData.orderNumber,
                    customerName: `User ${savedOrderData.userId}`,
                    totalAmount: savedOrderData.total,
                    timestamp: new Date(),
                });
                this.logger.log(`📡 SSE admin notification emitted for new order: ${savedOrderData.orderNumber}`);
            }
            return savedOrder;
        }
        catch (error) {
            console.error(...oo_tx(`3933823575_285_6_285_74_11`, '❌ Erreur lors de la création de la commande:', error));
            if (error instanceof common_1.BadRequestException ||
                error instanceof common_1.NotFoundException) {
                throw error;
            }
            throw new common_1.InternalServerErrorException('Erreur interne lors de la création de la commande');
        }
    }
    async findUserOrders(userId, page = 1, limit = 10) {
        if (!mongoose_2.Types.ObjectId.isValid(userId)) {
            throw new common_1.BadRequestException('ID utilisateur invalide');
        }
    ...
    async findOne(orderId, userId) {
        if (!mongoose_2.Types.ObjectId.isValid(orderId)) {
            throw new common_1.BadRequestException('ID de commande invalide');
        }
    ...
    async updateStatus(orderId, status, message) {
        if (!mongoose_2.Types.ObjectId.isValid(orderId)) {
            throw new common_1.BadRequestException('ID de commande invalide');
        }
        ...
        if (status === orders_schema_1.OrderStatus.SHIPPED) {
            updateData.shippedAt = new Date();
        }
        else if (status === orders_schema_1.OrderStatus.DELIVERED) {
        ...
        if (updatedOrder) {
            const statusMessages = {
                [orders_schema_1.OrderStatus.PENDING]: 'Commande en attente de paiement',
                [orders_schema_1.OrderStatus.PAID]: 'Paiement confirmé',
                [orders_schema_1.OrderStatus.CONFIRMED]: "Commande confirmée par l'équipe",
                [orders_schema_1.OrderStatus.PROCESSING]: 'Commande en préparation',
                [orders_schema_1.OrderStatus.SHIPPED]: 'Commande en cours de livraison',
                [orders_schema_1.OrderStatus.DELIVERED]: 'Commande livrée avec succès',
                [orders_schema_1.OrderStatus.CANCELLED]: 'Commande annulée',
                [orders_schema_1.OrderStatus.REFUNDED]: 'Commande remboursée',
            };
            ...
            this.logger.log(`📡 SSE event emitted for order ${updatedOrder.orderNumber}: ${status}`);
        }
        return updatedOrder;
    }
    ...
    async cancelOrder(orderId, reason = 'Annulation demandée') {
        if (!mongoose_2.Types.ObjectId.isValid(orderId)) {
            throw new common_1.BadRequestException('ID de commande invalide');
        }
        try {
            ...
            if ([
                orders_schema_1.OrderStatus.SHIPPED,
                orders_schema_1.OrderStatus.DELIVERED,
                orders_schema_1.OrderStatus.CANCELLED,
            ...
            const updatedOrder = await this.orderModel
                .findByIdAndUpdate(orderId, {
                status: orders_schema_1.OrderStatus.CANCELLED,
                updatedAt: new Date(),
                $push: {
                    statusHistory: {
                ...
                .exec();
            console.log(...oo_oo(`3933823575_612_6_612_76_4`, `✅ Commande ${orderData.orderNumber} annulée avec succès`));
            await this.sendOrderCancelledNotification(updatedOrder, reason);
            return updatedOrder;
        }
        catch (error) {
            console.error(...oo_tx(`3933823575_623_6_623_75_11`, "❌ Erreur lors de l'annulation de la commande:", error));
            throw error;
        }
    }
    sendOrderCreatedNotification(order) {
        try {
            console.log(...oo_oo(`3933823575_639_6_639_24_4`, order));
            const orderData = order;
            return orderData;
        }
        catch (error) {
            const err = error;
            console.error(...oo_tx(`3933823575_674_6_676_7_11`, `❌ Erreur lors de l'envoi de la notification de commande créée: ${err.message}`));
        }
    }
    async sendOrderCancelledNotification(order, reason) {
        try {
            const orderData = order;
            console.log(...oo_oo(`3933823575_690_6_690_25_4`, reason));
            ...
            if (!populatedOrder)
                return;
            console.log(...oo_oo(`3933823575_709_6_711_7_4`, `📧 Order cancelled notification skipped (template not implemented yet): ${orderData.orderNumber}`));
        }
        catch (error) {
            const err = error;
            console.error(...oo_tx(`3933823575_714_6_716_7_11`, `❌ Erreur lors de l'envoi de la notification d'annulation: ${err.message}`));
        }
    }
    ...
    async assignDriver(orderId, driverId, estimatedDelivery) {
        if (!mongoose_2.Types.ObjectId.isValid(orderId)) {
            throw new common_1.BadRequestException("L'ID de la commande n'est pas valide");
        }
        if (!mongoose_2.Types.ObjectId.isValid(driverId)) {
            throw new common_1.BadRequestException("L'ID du livreur n'est pas valide");
        }
        ...
        const updatedOrder = await this.orderModel
            ...
            estimatedDelivery,
            assignedAt: new Date(),
            status: orders_schema_1.OrderStatus.SHIPPED,
        }, { new: true })
            .populate('userId', 'name email')
            .populate('shippingAddress');
        if (!updatedOrder) {
            throw new common_1.InternalServerErrorException("Erreur lors de l'assignation du livreur");
        }
        try {
            const userData = updatedOrder.userId;
            const shippingData = updatedOrder.shippingAddress;
            ...
            this.logger.log(`📧 Email de livraison envoyé avec code ${validationCode} pour commande ${updatedOrder.orderNumber}`);
        }
        catch (error) {
            const err = error;
            this.logger.error(`❌ Erreur lors de l'envoi de l'email de livraison: ${err.message}`);
        }
        this.orderEventsService.emitOrderStatusUpdate({
            orderId: updatedOrder._id.toString(),
            status: orders_schema_1.OrderStatus.SHIPPED,
        ...
        this.logger.log(`✅ Livreur ${driverId} assigné à la commande ${order.orderNumber} avec code ${validationCode}`);
        return updatedOrder;
    }
    async getDriverOrders(driverId, filters) {
        ...
        const orders = await this.orderModel
            ...
            .populate('shippingAddress')
            .sort({ assignedAt: -1 });
        this.logger.log(`📦 Récupération de ${orders.length} commandes pour le livreur ${driverId}`);
        return orders;
    }
    async validateDelivery(orderId, driverId, validationCode, deliveryNote, signatureUrl) {
        if (!mongoose_2.Types.ObjectId.isValid(orderId)) {
            throw new common_1.BadRequestException("L'ID de la commande n'est pas valide");
        }
        const order = await this.orderModel.findById(orderId);
        ...
        if (!order.driverId || order.driverId.toString() !== driverId) {
            throw new common_1.BadRequestException("Vous n'êtes pas assigné à cette commande");
        }
        if (order.validationCode !== validationCode.toUpperCase()) {
            throw new common_1.BadRequestException('Code de validation incorrect');
        }
        if (order.status !== orders_schema_1.OrderStatus.SHIPPED) {
            throw new common_1.BadRequestException('Cette commande ne peut pas être validée (statut incorrect)');
        }
        const updatedOrder = await this.orderModel.findByIdAndUpdate(orderId, {
            status: orders_schema_1.OrderStatus.DELIVERED,
            deliveryNote,
            signatureUrl,
            deliveredAt: new Date(),
        }, { new: true });
        if (!updatedOrder) {
            throw new common_1.InternalServerErrorException('Erreur lors de la validation de la livraison');
        }
        this.orderEventsService.emitOrderStatusUpdate({
            orderId: updatedOrder._id.toString(),
            status: orders_schema_1.OrderStatus.DELIVERED,
        ...
        this.logger.log(`✅ Livraison validée pour commande ${order.orderNumber} par livreur ${driverId}`);
        return updatedOrder;
    }
};
