...
export const listBanners = async (query) => {
    const filters = [];
    if (query.placement)
        filters.push(eq(t.banners.placement, query.placement));
    if (query.status)
        filters.push(eq(t.banners.status, query.status));
 ...
 */
export const listPublicBanners = async (placement) => {
    const now = new Date();
    const filters = [
...
export const listStaticPages = async (query) => {
    const filters = [];
    if (query.status)
        filters.push(eq(t.staticPages.status, query.status));
    if (query.q)
        filters.push(ilike(t.staticPages.title, `%${query.q}%`));
    const where = filters.length > 0 ? and(...filters) : undefined;
    const { limit, offset } = toSqlPagination(query);
    const [rows, [totals]] = await Promise.all([
        db
...
export const getStaticPageBySlug = async (slug, options = {}) => {
    const filters = [eq(t.staticPages.slug, slug)];
    if (options.publishedOnly)
        filters.push(eq(t.staticPages.status, "published"));
    const [row] = await db
        .select()
        .from(t.staticPages)
        .where(and(...filters))
...
export const createStaticPage = async (input, userId) => {
...
export const updateStaticPage = async (id, input, userId) => {
    const [current] = await db
...
// --- Mises en avant --------------------------------------------------------
export const listFeaturedEntries = async (sectionKey, options = {}) => {
    const filters = [eq(t.featuredEntries.isActive, true)];
    if (sectionKey)
        filters.push(eq(t.featuredEntries.sectionKey, sectionKey));
    const rows = await db
        .select()
    ...
    const [products, categories] = await Promise.all([
        productIds.length > 0
            ? db
                ...
                .from(t.products)
                .where(and(sql `${t.products.id} in ${productIds}`, eq(t.products.status, "published"), isNull(t.products.deletedAt)))
            : Promise.resolve([]),
        categoryIds.length > 0
            ? db
                ...
                .from(t.categories)
                .where(and(sql `${t.categories.id} in ${categoryIds}`, isNull(t.categories.deletedAt)))
            : Promise.resolve([]),
    ]);
...
export const createFeaturedEntry = async (input) => {
    const [created] = await db
...
export const deleteFeaturedEntry = async (id) => {
    const [deleted] = await db
 ...
 * contourné.
 */
export const createContactMessage = async (input, context) => {
    const [created] = await db
...
export const countNewContactMessages = async () => {
    const [row] = await db
        .select({ total: count() })
        .from(t.contactMessages)
