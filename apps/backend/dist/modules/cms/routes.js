...
adminCmsRoutes.get("/banners", requirePermission(PERMISSIONS.content.read), validate("query", bannerListQuerySchema), async (c) => c.json(await service.listBanners(c.req.valid("query"))));
adminCmsRoutes.post("/banners", requirePermission(PERMISSIONS.content.write), validate("json", bannerInputSchema), async (c) => {
...
adminCmsRoutes.patch("/banners/:id", requirePermission(PERMISSIONS.content.write), validate("param", idParam), 
...
validate("json", updateBannerSchema), async (c) => {
...
adminCmsRoutes.delete("/banners/:id", requirePermission(PERMISSIONS.content.write), validate("param", idParam), async (c) => {
    const { id } = c.req.valid("param");
...
adminCmsRoutes.get("/pages", requirePermission(PERMISSIONS.content.read), validate("query", staticPageListQuerySchema), async (c) => c.json(await service.listStaticPages(c.req.valid("query"))));
adminCmsRoutes.post("/pages", requirePermission(PERMISSIONS.content.write), validate("json", staticPageInputSchema), async (c) => {
...
adminCmsRoutes.patch("/pages/:id", requirePermission(PERMISSIONS.content.write), validate("param", idParam), validate("json", staticPageInputSchema.partial()), async (c) => {
    const { id } = c.req.valid("param");
    const updated = await service.updateStaticPage(id, c.req.valid("json"), currentUser(c).sub);
    await recordAudit(c, {
        action: "page.updated",
        resourceType: "static_page",
        resourceId: id,
...
adminCmsRoutes.delete("/pages/:id", requirePermission(PERMISSIONS.content.write), validate("param", idParam), async (c) => {
    const { id } = c.req.valid("param");
...
adminCmsRoutes.get("/featured", requirePermission(PERMISSIONS.content.read), validate("query", z.object({ sectionKey: z.string().max(64).optional() })), async (c) => c.json(await service.listFeaturedEntries(c.req.valid("query").sectionKey)));
adminCmsRoutes.post("/featured", requirePermission(PERMISSIONS.content.write), validate("json", featuredEntryInputSchema), async (c) => {
...
adminCmsRoutes.delete("/featured/:id", requirePermission(PERMISSIONS.content.write), validate("param", idParam), async (c) => {
    const { id } = c.req.valid("param");
