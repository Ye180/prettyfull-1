...
import {
	getShippingAdapter,
	listShippingAdapters,
} from "../../integrations/shipping/registry.js";
/**
...
export const testPaymentProvider = async (key) => {
	...
	if (!adapter.test) {
		return {
			ok: true,
			message: "Cet agrégateur ne propose pas de test de connexion.",
		};
	}
	const [row] = await db
		.select()
...
export const createShippingRate = async (input) => {
	if (!getShippingAdapter(input.providerKey)) {
		throw badRequest("Transporteur inconnu.", {
			providerKey: ["Aucun adaptateur enregistré."],
		});
	}
