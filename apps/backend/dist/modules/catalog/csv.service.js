...
export const exportProductsCsv = async () => {
	const rows = await db
		...
		.leftJoin(
			t.productImages,
			and(
				eq(t.productImages.productId, t.products.id),
				eq(t.productImages.position, 0),
			),
		)
		.where(isNull(t.products.deletedAt))
		.orderBy(
			asc(t.products.name),
			asc(t.productVariants.position),
			asc(t.sizes.position),
		);
...
export const importProductsCsv = async (csv, userId, dryRun = false) => {
	...
	if (lines.length < 2) {
		report.errors.push({
			line: 0,
			slug: "",
			message: "Fichier vide ou sans ligne de données.",
		});
		return report;
	}
	...
	for (const [index, line] of lines.slice(1).entries()) {
		...
		if (!slug) {
			report.errors.push({
				line: index + 2,
				slug: "",
				message: "Colonne « slug » manquante.",
			});
			continue;
		}
	...
	for (const [slug, rows] of groups) {
		...
		try {
			...
			const kind =
				(cells.regime || "").toLowerCase() === "variant" ? "variant" : "simple";
