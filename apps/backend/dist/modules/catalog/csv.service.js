...
export const exportProductsCsv = async () => {
    const rows = await db
        ...
        .leftJoin(t.productCategories, and(eq(t.productCategories.productId, t.products.id), eq(t.productCategories.isPrimary, true)))
        .leftJoin(t.categories, eq(t.categories.id, t.productCategories.categoryId))
        .leftJoin(t.productImages, and(eq(t.productImages.productId, t.products.id), eq(t.productImages.position, 0)))
 ...
 */
export const importProductsCsv = async (csv, userId, dryRun = false) => {
    const report = { created: 0, updated: 0, skipped: 0, errors: [], dryRun };
    ...
    const categoryByName = new Map(categoryRows.flatMap((row) => [
        [row.name.toLowerCase(), row.id],
        [row.slug.toLowerCase(), row.id],
    ]));
        ...
        try {
            ...
            for (const row of rows) {
                ...
                if (kind === "variant" && variantName) {
                    ...
                    if (sizeLabel)
                        variant.sizes.push({ label: sizeLabel, initialQuantity: quantity });
                    else
                        variant.initialQuantity = quantity;
