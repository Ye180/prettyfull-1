...
const loadProduct = async (productId) => {
	...
	if (product.archivedAt) {
		throw conflict(
			"Ce produit est archivé. Restaurez-le avant de le modifier.",
		);
	}
	return product;
};
		...
		if (input.sizes.length > 0) {
			for (const [index, size] of input.sizes.entries()) {
				...
				await tx.insert(t.inventoryItems).values({
					...
					lowStockThreshold:
						size.lowStockThreshold ?? product.lowStockThreshold,
				});
			}
		} else {
			await tx.insert(t.inventoryItems).values({
...
export const updateVariant = async (productId, variantId, input) => {
	const [existing] = await db
		...
		.where(
			and(
				eq(t.productVariants.id, variantId),
	...
	await db.transaction(async (tx) => {
		...
		if (images) {
			await tx
				.delete(t.variantImages)
				.where(eq(t.variantImages.variantId, variantId));
			if (images.length > 0) {
				await tx.insert(t.variantImages).values(
