...
// --- Variantes -------------------------------------------------------------
export const addVariant = async (productId, input) => {
    const product = await loadProduct(productId);
    if (product.kind !== "variant") {
        throw invalidProductModel("Ce produit est déclaré « simple » : il ne peut pas recevoir de variantes. Dupliquez-le en produit à variantes.");
    }
    await db.transaction(async (tx) => {
        ...
        if (input.images.length > 0) {
            await tx.insert(t.variantImages).values(input.images.map((image, index) => ({
                variantId,
                url: image.url,
                alt: image.alt ?? null,
...
export const updateVariant = async (productId, variantId, input) => {
    const [existing] = await db
        .select({ id: t.productVariants.id })
        .from(t.productVariants)
        .where(and(eq(t.productVariants.id, variantId), eq(t.productVariants.productId, productId)))
        .limit(1);
    if (!existing)
    ...
    await db.transaction(async (tx) => {
        ...
        if (images) {
            ...
            if (images.length > 0) {
                await tx.insert(t.variantImages).values(images.map((image, index) => ({
                    variantId,
                    url: image.url,
                    alt: image.alt ?? null,
                    position: image.position ?? index,
...
export const deleteVariant = async (productId, variantId) => {
    ...
    if (product.variants.length <= 1) {
        throw conflict("Un produit à variantes doit conserver au moins une variante. Archivez le produit si besoin.");
    }
    const sold = await db
 ...
 */
export const addSize = async (productId, input) => {
    const product = await loadProduct(productId);
    if (product.kind === "variant" && !input.variantId) {
        throw invalidProductModel("Ce produit est à variantes : la taille doit être rattachée à une variante.");
    }
    if (product.kind === "simple" && input.variantId) {
        throw invalidProductModel("Ce produit est simple : ses tailles se rattachent au produit, pas à une variante.");
    }
    ...
    await db.transaction(async (tx) => {
        ...
        if (input.variantId) {
            await tx
                ...
                .where(and(eq(t.inventoryItems.productId, productId), eq(t.inventoryItems.variantId, input.variantId), isNull(t.inventoryItems.sizeId)));
        }
        else {
            await tx
                .delete(t.inventoryItems)
                .where(and(eq(t.inventoryItems.productId, productId), isNull(t.inventoryItems.variantId), isNull(t.inventoryItems.sizeId)));
        }
    });
    return getProduct(productId);
};
export const updateSize = async (productId, sizeId, input) => {
 ...
 */
export const deleteSize = async (productId, sizeId) => {
