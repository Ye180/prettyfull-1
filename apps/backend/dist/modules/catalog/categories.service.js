...
const resolvePlacement = async (slug, parentId) => {
	...
	const [parent] = await db
		...
		.limit(1);
	if (!parent)
		throw badRequest("Catégorie parente introuvable.", {
			parentId: ["Inconnue."],
		});
	if (parent.depth + 1 > MAX_DEPTH) {
		throw badRequest(`Profondeur maximale atteinte (${MAX_DEPTH} niveaux).`, {
		...
		if (input.parentId) {
			...
			if (
				...
				(target.path === current.path ||
					target.path.startsWith(`${current.path}/`))
			) {
				throw badRequest(
...
export const archiveCategory = async (id) => {
	...
	const [products] = await db
		...
		.where(
			and(eq(t.productCategories.categoryId, id), isNull(t.products.deletedAt)),
		);
...
export const countProductsByCategory = async () => {
	const rows = await db
		...
		.innerJoin(t.products, eq(t.products.id, t.productCategories.productId))
		.where(
			and(isNull(t.products.deletedAt), eq(t.products.status, "published")),
		)
...
export const getDescendantIds = async (categoryId) => {
	...
	const rows = await db
		...
		.where(
			and(
				like(t.categories.path, `${root.path}%`),
