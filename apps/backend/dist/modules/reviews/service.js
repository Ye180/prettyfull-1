 ...
 */
export const createReview = async (input, context) => {
...
export const getReviewSummary = async (productId) => {
    const [row] = await db
        .select({
        average: sql `avg(${t.reviews.rating})`,
        count: count(),
    })
        .from(t.reviews)
...
export const listReviewsAdmin = async (query) => {
    const filters = [];
    if (query.productId)
        filters.push(eq(t.reviews.productId, query.productId));
    if (query.status)
        filters.push(eq(t.reviews.status, query.status));
