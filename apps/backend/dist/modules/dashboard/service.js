	...
	const [
		...
		db
			...
			.where(
				and(
					paidOrders,
					gte(t.orders.createdAt, previousStart),
					lt(t.orders.createdAt, periodStart),
				),
			),
		db
			.select({
		...
		db
			...
			.from(t.users)
			.where(
				and(eq(t.users.kind, "customer"), gte(t.users.createdAt, periodStart)),
			),
		db
			.select({ total: count() })
			.from(t.users)
		...
		db
			...
			.from(t.orders)
			.where(
				inArray(t.orders.status, ["pending_payment", "paid", "preparing"]),
			),
		db
			.select({
