...
export const getDashboard = async (periodDays) => {
    ...
    const [[currentPeriod], [previousPeriod], [today], [newCustomers], [previousCustomers], [pending], [stockCounts], series, recentOrders, lowStockAlerts, topProducts,] = await Promise.all([
        db
            .select({
            revenue: sql `coalesce(sum(${t.orders.total} - ${t.orders.refundedTotal}), 0)::int`,
            orders: count(),
        })
            .from(t.orders)
        ...
        db
            ...
            .from(t.orders)
            .where(and(paidOrders, gte(t.orders.createdAt, previousStart), lt(t.orders.createdAt, periodStart))),
        db
            .select({
            revenue: sql `coalesce(sum(${t.orders.total} - ${t.orders.refundedTotal}), 0)::int`,
            orders: count(),
        ...
        db
            .select({ total: count() })
            .from(t.users)
            .where(and(eq(t.users.kind, "customer"), gte(t.users.createdAt, previousStart), lt(t.users.createdAt, periodStart))),
        db
            .select({ total: count() })
            .from(t.orders)
        ...
        db
            ...
            customerName: sql `nullif(trim(coalesce(${t.users.firstName}, '') || ' ' || coalesce(${t.users.lastName}, '')), '')`,
        ...
        db
            ...
            name: sql `max(${t.orderItems.productName})`,
            thumbnail: sql `max(${t.orderItems.thumbnail})`,
            unitsSold: sql `sum(${t.orderItems.quantity})::int`,
            revenue: sql `sum(${t.orderItems.lineTotal})::int`,
        })
            .from(t.orderItems)
    ...
    const previousAverage = previousOrderCount > 0 ? Math.round(previousRevenue / previousOrderCount) : 0;
    return {
        ...
        topProducts: topProducts.flatMap((row) => row.productId
            ? [
                {
                    productId: row.productId,
                    name: row.name,
