...
const assertCodeAvailable = async (code, excludeId) => {
...
export const listPromoCodes = async (query) => {
    const filters = [];
    if (query.status)
        filters.push(eq(t.promoCodes.status, query.status));
    if (query.q) {
        filters.push(sql `(
...
export const updatePromoCode = async (id, input) => {
    if (input.code)
 ...
 */
export const resolveDiscountForSubtotal = async (code, subtotal) => {
    ...
    if (promo.minOrderAmount != null && subtotal < promo.minOrderAmount) {
        throw badRequest(`Ce code promo s'applique à partir de ${promo.minOrderAmount} dans la devise du panier.`, { code: ["Montant minimum non atteint."] });
    }
    return {
 ...
 */
export const getAppliedDiscount = async (promoCodeId, subtotal) => {
    const [promo] = await db
        .select()
        .from(t.promoCodes)
...
export const incrementUsage = async (id, executor = db) => {
    await executor
        .update(t.promoCodes)
