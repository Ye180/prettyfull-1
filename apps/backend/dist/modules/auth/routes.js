...
const buildAuthRoutes = (kind, scope) => {
	...
	app.post("/refresh", validate("json", refreshSchema), async (c) => {
		const token =
			readRefreshCookie(c, scope) ?? c.req.valid("json").refreshToken;
		if (!token) throw unauthorized("Aucune session à renouveler.");
		const { auth, refreshToken } = await service.refresh(
			token,
			kind,
			context(c),
		);
	...
	app.patch(
		...
		async (c) =>
			c.json(
				await service.updateProfile(currentUser(c).sub, c.req.valid("json")),
			),
	);
	/**
