...
const errorResponses = {
	...
	409: {
		description: "Conflit métier (stock, transition, unicité)",
		...json(ref("ApiError")),
	},
};
	...
	tags: [
		{
			...
			description: "Sessions client et back-office.",
		},
		...
		{
			name: "Contenu",
			description: "Bannières, pages statiques, mises en avant.",
		},
		{
			name: "Administration",
			description: "Comptes, rôles, paramètres, audit.",
		},
	],
	components: {
		securitySchemes: {
		...
		schemas: {
			...
			Money: {
				type: "integer",
				description:
					"Montant entier dans la plus petite unité de la devise (58500 = 58 500 FCFA).",
			},
			Product: {
				type: "object",
				properties: {
					...
					kind: {
						...
						enum: [...PRODUCT_KINDS],
						description:
							"`simple` : tailles portées par le produit. `variant` : tailles portées par les coloris. Les deux ne coexistent jamais.",
					},
					name: { type: "string" },
					slug: { type: "string" },
					basePrice: ref("Money"),
			...
			Order: {
				...
				properties: {
					...
					displayId: {
						type: "integer",
						description: "Numéro lisible communiqué au client.",
					},
					...
					items: {
						type: "array",
						description:
							"Instantané figé à l'achat : libellés et prix ne suivent pas les modifications ultérieures du catalogue.",
						items: { type: "object" },
					},
				},
			},
			InventoryRow: {
				...
				properties: {
					...
					quantity: {
						type: "integer",
						description: "Stock physique, réservations comprises.",
					},
					reservedQuantity: {
						type: "integer",
	...
	paths: {
		...
		"/api/store/auth/login": {
			post: {
				...
				description:
					"Un compte back-office est refusé ici, et réciproquement : les deux portails sont cloisonnés par le type de compte.",
				security: [],
				requestBody: json({
					type: "object",
				...
				responses: {
					200: {
						description: "Session ouverte ; cookie de rafraîchissement posé",
					},
					...errorResponses,
				},
			},
		...
		"/api/admin/auth/refresh": {
			post: {
				...
				security: [],
				responses: {
					200: { description: "Nouvelle session" },
					...errorResponses,
				},
			},
		},
		...
		"/api/store/products": {
			get: {
				...
				description:
					"Seuls les produits publiés sont servis : aucun paramètre ne permet d'atteindre un brouillon.",
				security: [],
				parameters: [
					...paginationParams,
					{
						...
						in: "query",
						schema: { type: "string" },
						description: "Recherche plein texte, insensible aux accents.",
					},
		...
		"/api/store/products/{slug}": {
			get: {
				...
				responses: {
					200: { description: "Produit", ...json(ref("Product")) },
					...errorResponses,
				},
			},
		},
				...
				parameters: [
					{
						...
						in: "query",
						schema: { type: "boolean" },
						description: "Renvoie l'arbre complet plutôt qu'une page plate.",
					},
				],
		...
		"/api/store/categories/{slug}/products": {
			get: {
				...
				description:
					"Le paramètre accepte un slug ou un identifiant. Les sous-rayons sont inclus.",
				security: [],
				parameters: [
					{
						name: "slug",
		...
		"/api/store/cart": {
			get: {
				...
				description:
					"Panier de la cliente connectée, ou du visiteur via son cookie de panier.",
				security: [],
		...
		"/api/store/cart/items": {
			post: {
				...
				description:
					"Le triplet produit/variante/taille désigne le point de stock. La disponibilité est vérifiée à l'ajout.",
				security: [],
				requestBody: json({
					type: "object",
					required: ["productId"],
				...
				responses: {
					...
					409: {
						description:
							"Stock insuffisant ; `details` porte le disponible réel",
						...json(ref("ApiError")),
					},
				},
			},
		...
		"/api/store/cart/shipping-options": {
			get: {
				...
				description:
					"Calculées par les adaptateurs de livraison actifs, d'après l'adresse et le poids du panier.",
				security: [],
				responses: {
					200: { description: "Options disponibles" },
					...errorResponses,
				},
			},
		},
		...
		"/api/store/orders/{id}/confirmation": {
			get: {
				...
				responses: {
					200: { description: "Commande", ...json(ref("Order")) },
					...errorResponses,
				},
			},
		},
		...
		"/api/admin/dashboard": {
			get: {
				...
				description:
					"Permission `dashboard.read`. Ne comptabilise que les commandes payées.",
				parameters: [
					{
						name: "period",
						in: "query",
				...
				responses: {
					200: { description: "Indicateurs et alertes" },
					...errorResponses,
				},
			},
		},
		"/api/admin/products": {
			get: {
				...
				responses: {
					200: { description: "Page de produits" },
					...errorResponses,
				},
			},
			post: {
				...
				responses: {
					...
					...errorResponses,
					422: {
						description: "Modèle de produit incohérent",
						...json(ref("ApiError")),
					},
				},
			},
		...
		"/api/admin/products/import": {
			post: {
				...
				summary: "Import CSV",
				description:
					"Permission `catalog.import`. `dryRun` simule sans rien écrire.",
				responses: {
			...
			get: {
				...
				parameters: [
					...
					{ name: "lowStockOnly", in: "query", schema: { type: "boolean" } },
					{
						name: "stockStatus",
						in: "query",
						schema: { type: "string", enum: [...STOCK_STATUSES] },
					},
				],
				responses: {
					200: {
			...
			post: {
				...
				requestBody: json({
					...
					properties: {
						...
						delta: {
							type: "integer",
							description: "Signé. `-3` pour une casse.",
						},
		...
		"/api/admin/orders": {
			get: {
				...
				description:
					"Permission `orders.read`. La recherche accepte un numéro, un e-mail ou un suivi.",
				parameters: paginationParams,
				responses: {
					200: { description: "Page de commandes" },
					...errorResponses,
				},
			},
		},
		"/api/admin/orders/{id}/status": {
			patch: {
				...
				description:
					"Permission `orders.write`. Seules les transitions autorisées passent ; annuler libère les réservations et remet en stock ce qui avait été décrémenté.",
				parameters: [
					{
						name: "id",
						in: "path",
		...
		"/api/admin/orders/{id}/refund": {
			post: {
				...
				summary: "Rembourser",
				description:
					"Permission `orders.refund`. Total ou partiel, avec remise en stock optionnelle. Le prestataire est sollicité avant toute écriture.",
				parameters: [
					{
						name: "id",
						in: "path",
		...
		"/api/admin/orders/{id}/invoice": {
			get: {
				...
				summary: "Facture",
				description:
					"Permission `orders.read`. HTML imprimable en PDF depuis le navigateur.",
				parameters: [
					{
						name: "id",
						in: "path",
		...
		"/api/admin/integrations/payment": {
			get: {
				...
				description:
					"Permission `integrations.read`. Les clés ne sont **jamais** renvoyées : seule la liste des clés renseignées l'est.",
				responses: {
		...
		"/api/admin/integrations/payment/{key}": {
			patch: {
				...
				description:
					"Permission `integrations.write`. L'activation est refusée tant qu'une clé obligatoire manque.",
				parameters: [
					{
						name: "key",
						in: "path",
			...
			get: {
				...
				parameters: [
					...paginationParams,
					{
						name: "placement",
						in: "query",
						schema: { type: "string", enum: [...BANNER_PLACEMENTS] },
					},
				],
		...
		"/api/admin/audit-logs": {
			get: {
				...
				description:
					"Permission `audit.read`. Trace des actions sensibles (§5).",
				parameters: paginationParams,
				responses: {
					200: { description: "Entrées du journal" },
					...errorResponses,
				},
			},
		},
