...
const ProductSuggestion = ({ categorySlug, excludeProductId }: ProductSuggestionProps) => {
	const currencyCode = useRegionStore((state) => state.region?.currency_code);
	const { data: products, isLoading } = useRelatedProducts(categorySlug, excludeProductId);
				...
				<>
					{isLoading
						? Array.from({ length: 4 }).map((_, index) => (
								<div key={index} className="w-full aspect-3/4 bg-gray-100 rounded-[2.2rem] animate-pulse" />
							))
